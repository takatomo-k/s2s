...
class BeamSearch(DecodeStrategy):
    ...
    def __init__(self, beam_size, batch_size, n_best, mb_device,
                 global_scorer, pad, bos, eos, min_length, max_length, return_attention,
                 block_ngram_repeat, exclusion_tokens, memory_lengths,
                 stepwise_penalty, ratio):
        super(BeamSearch, self).__init__(
            pad, bos, eos,
            batch_size, mb_device, beam_size, min_length,
        ...
        self.topk_ids = torch.empty((batch_size, beam_size), dtype=torch.long,
                                    device=mb_device)
        self.stops = torch.empty((batch_size, beam_size), dtype=torch.long,
                                    device=mb_device) 
    ...
    def advance(self, log_probs, attn, find_eos, context):
        ...
        if self.return_attention or self._cov_pen:
            ...
            current_context = context.index_select(1, self.select_indices)
            current_probs = log_probs.unsqueeze(0).index_select(1, self.select_indices)
            if step == 1:
                self.alive_attn = current_attn
                self.alive_context = current_context
                self.alive_probs = current_probs
                ...
                self.alive_context = self.alive_context.index_select(
                    1, self.select_indices)
                self.alive_probs = self.alive_probs.index_select(
                    1, self.select_indices)
                self.alive_attn = torch.cat([self.alive_attn, current_attn], 0)
                self.alive_context = torch.cat([self.alive_context, current_context], 0)
                self.alive_probs = torch.cat([self.alive_probs, current_probs], 0)
                # update global state (step > 1)
                if self._cov_pen:
        ...
        self.is_finished = find_eos(self.topk_ids)
    ...
    def update_finished(self):
        ...
        #self.top_beam_finished |= self.is_finished[-1 ].eq(1)
        ...
        context = (
            self.alive_context.view(
                step - 1, _B_old, self.beam_size, self.alive_context.size(-1))
            if self.alive_context is not None else None)
        probs = (
            self.alive_probs.view(
                step - 1, _B_old, self.beam_size, self.alive_probs.size(-1))
            if self.alive_probs is not None else None)
        non_finished_batch = []
        for i in range(self.is_finished.size(0)):
            ...
            if finish_flag and len(self.hypotheses[b]) >= self.n_best:
                ...
                for n, (score, pred, attn, cont, prob) in enumerate(best_hyp):
                    if n >= self.n_best:
                        break
