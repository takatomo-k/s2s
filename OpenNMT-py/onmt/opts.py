...
def model_opts(parser):
    ...
    group.add('--share_dec_emb', '-share_dec_emb',
              action='store_true',
              help="Use a shared weight matrix for the input and "
                   "output word  embeddings in the decoder.")
    
    group.add('--share_emb', '-share_emb', action='store_true',
    ...
    group.add('--encoder_type', '-encoder_type', type=str, default='rnn',
              #choices=['rnn', 'brnn', 'mean', 'transformer', 'cnn', 'audio'],
    ...
    group.add('--layers', '-layers', type=int, default=3,
    ...
    group.add('--enc_pooling', '-enc_pooling',
              type=str, default='1',
    ...
    group.add('--adversarial', '-adversarial', action="store_true",
               help="Have an additional layer between the last encoder "
                    "state and the first decoder state")
    ...
    group.add('--gen_type', '-gen_type', type=int, default=0,
              help='Size of hidden transformer feed-forward')
    group.add('--noisy_enc', '-noisy_enc', type=float, default=0,
              help='Train copy attention layer.')
    group.add('--noisy_dec', '-noisy_dec', type=float, default=0,
              help='Train copy attention layer.')
    # Generator and loss options.
    group.add('--copy_attn', '-copy_attn', action="store_true",
              help='Train copy attention layer.')
    group.add('--spkr', '-spkr', action="store_true",
              help='Train copy attention layer.')
...
def preprocess_opts(parser):
    ...
    group.add('--lang', '-lang', default=[], nargs='*', type=str,
               required=True, help="list of ranks of each process.")
    
    group.add('--domain', '-domain', default=[], nargs='*', type=str,
               required=True, help="list of ranks of each process.")

    group.add('--name', '-name', default=['src','tgt'], nargs='*', type=str,
               required=True, help="list of ranks of each process.")
               
    group.add('--train', '-train', default=[], nargs='*', type=str,
               required=True, help="list of ranks of each process.")

    group.add('--valid', '-valid', default=[], nargs='*', type=str,
               help="list of ranks of each process.")
    
    group.add('--test', '-test', default=[], nargs='*', type=str,
               help="list of ranks of each process.")

    group.add('--save_data', '-save_data', required=False,
    ...
    group.add('--src_words_min_frequency',
              '-src_words_min_frequency', type=int, default=3)
    group.add('--tgt_words_min_frequency',
              '-tgt_words_min_frequency', type=int, default=3)
...
def train_opts(parser):
    ...
    group.add('--save_model', '-save_model', default='exp/model/',
    ...
    group.add('--reset_optim', '-reset_optim', default='none',
              choices=['none', 'all', 'states', 'keep_states','score'],
    ...
    group.add('--add_word_vecs_enc', '-add_word_vecs_enc',
              help="If a valid path is specified, then this will load "
                   "pretrained word embeddings on the encoder side. "
                   "See README for specific formatting instructions.")
    group.add('--add_word_vecs_dec', '-add_word_vecs_dec',
              help="If a valid path is specified, then this will load "
                   "pretrained word embeddings on the decoder side. "
                   "See README for specific formatting instructions.")
    ...
    group.add('--fix_word_vecs_gen', '-fix_word_vecs_gen',
              action='store_true',
              help="Fix word embeddings on the generator side.")

    # Optimization options
    group = parser.add_argument_group('Optimization- Type')
    group.add('--batch_size', '-batch_size', type=int, default=128,
    ...
    group.add('--test_steps', '-test_steps', type=int, default=10000,
              help='Perfom validation every X steps')
    ...
    group.add('--add_vocoder', '-add_vocoder',
              help="If a valid path is specified, then this will load "
                   "pretrained word embeddings on the encoder side. "
                   "See README for specific formatting instructions.")
    # learning rate
    ...
    # multi task
    group = parser.add_argument_group('Multi Task')
    group.add('--model_type', '-model_type',
              default='single',
              #choices=['single', 'cascade', 'osamura', 'transcode', 'transcode2','spaec','google','ncascade'],
              help='Word embedding size for src.')
    group.add('--pretrain', '-pretrain',
              nargs='+', default=[None],
              help="In case there are several corpora.")
