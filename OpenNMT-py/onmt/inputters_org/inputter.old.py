...
def get_fields(
    ...
    src_field_kwargs = {"n_feats": nfeats[0],
                        "include_lengths": True,
                        "pad": pad, "bos": bos, "eos": eos,
                        "truncate": src_truncate,
                        "base_name": "src"}
    
    fields["src"] = fields_getters(data_list[0])(**src_field_kwargs)
    
    tgt_field_kwargs = {"n_feats": nfeats[1],
                        "include_lengths": True,
                        "pad": pad, "bos": bos, "eos": eos,
                        "truncate": tgt_truncate,
                        "base_name": "tgt"}

    fields["tgt"] = fields_getters(data_list[1])(**tgt_field_kwargs)
...
def build_dataset_iter(corpus_type, fields, opt, batch_size, is_train=True, multi=False):
    ...
    """
    if courpus_type =="test":
        dataset_paths = list(sorted(
                glob.glob('exp/data/bpe2char.ja-en.test.[0-9]*.pt')))
        fields=torch.load("exp/data/bpe2char.ja-en.vocab.pt")
    else:
        dataset_paths = list(sorted(
